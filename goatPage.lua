...
local function onPressBack( self,event )	
  ...
  scrollScreen:insert(backButton)
     ...
     for row in db:nrows("SELECT description,image FROM farmanimalInfo WHERE id = '2'") do


      local goatImage = display.newImage(row.image)
      goatImage.x = display.contentCenterX       
      goatImage.y =  display.viewableContentHeight/1.8
      goatImage.height = display.viewableContentWidth -  display.viewableContentWidth/16
      goatImage.width = display.viewableContentWidth -  2*display.viewableContentWidth/16
      sceneGroup:insert(goatImage)
      scrollScreen:insert(goatImage)

      local options =
       {
        text = row.description, 
        x = display.contentCenterX  ,
        y =  goatImage.y+ (goatImage.height/2) + (goatImage.height/4) + display.viewableContentHeight/2.7,
...
local function  playAudio( event )
    -- loading audio
    for row in db:nrows("SELECT audio FROM farmanimalInfo WHERE id = '2'") do
...
{  
    ...
    defaultFile = "goatTitle.png",
    overFile = "goatTitle.png"
}
...
local function playVideo( event )
  ...
  media.playVideo( "rtsp://r6---sn-5hn7su7z.c.youtube.com/CiILENy73wIaGQkBmsntrIxauxMYDSANFEgGUgZ2aWRlb3MM/0/0/0/video.3gp", media.RemoteSource, true )  
  -- body
end
...
local function  playSound( event )
    -- loading audio
    for row in db:nrows("SELECT sounds FROM farmanimalInfo WHERE id = '2'") do
