...
local function onPressBack( self,event )	
  ...
  scrollScreen:insert(backButton)
     ...
     for row in db:nrows("SELECT description,image FROM domesticanimal WHERE id = '4'") do

      local rabbitImage = display.newImage(row.image)
      rabbitImage.x = display.contentCenterX       
      rabbitImage.y =   display.viewableContentHeight/1.8 
      rabbitImage.height = display.viewableContentWidth -  display.viewableContentWidth/16
      rabbitImage.width = display.viewableContentWidth -  2*display.viewableContentWidth/16
      sceneGroup:insert(rabbitImage)
      scrollScreen:insert(rabbitImage)

      local options =
       {
        text = row.description, 
        x = display.contentCenterX +  display.contentCenterX /16 ,
        y = rabbitImage.y+ (rabbitImage.height/2) + (rabbitImage.height/4) + display.viewableContentHeight/2.7,
        width = display.viewableContentWidth - display.viewableContentWidth/8, 
        height = display.viewableContentHeight/2,
        font = "Verdana",
        fontSize = 20,
        align =  center
      }
...
local function onPressFlipBackward( event )  
    composer.gotoScene( "horsePage", "flip", 200  )   
 ...
 local function onPressFlipForward( event )  
    composer.gotoScene( "dogPage", "flip", 200  )
...
local function playVideo( event )
  ...
  media.playVideo( "rtsp://r7---sn-5hn7su7s.c.youtube.com/CiILENy73wIaGQkLNce0dQiurRMYDSANFEgGUgZ2aWRlb3MM/0/0/0/video.3gp", media.RemoteSource, true )  
  -- body
end
...
local function  playAudio( event )
    -- loading audio
    for row in db:nrows("SELECT audio FROM domesticanimal WHERE id = '4'") do
...
{  
    ...
    defaultFile = "rabbitTitle.png",
    overFile = "rabbitTitle.png"
}
...
local function  playSound( event )
    -- loading audio
    for row in db:nrows("SELECT sounds FROM domesticanimal WHERE id = '4'") do
