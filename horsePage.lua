...
local function onPressBack( self,event )	
    ...
    composer.removeScene("horsePage")
  ...
  scrollScreen:insert(backButton)
     ...
     for row in db:nrows("SELECT description,image FROM domesticanimal WHERE id = '3'") do

      local horseImage = display.newImage(row.image)
      horseImage.x = display.contentCenterX       
      horseImage.y =  display.viewableContentHeight/1.8
      horseImage.height = display.viewableContentWidth -  display.viewableContentWidth/16
      horseImage.width = display.viewableContentWidth -  2*display.viewableContentWidth/16
      sceneGroup:insert(horseImage)
      scrollScreen:insert(horseImage)

      local options =
       {
        text = row.description, 
        x = display.contentCenterX +  display.contentCenterX /16 ,
        y = horseImage.y+ (horseImage.height/2) + (horseImage.height/4) + display.viewableContentHeight/2.7,
...
local function onPressFlipBackward( event )  
    composer.gotoScene( "catPage", "flip", 200  )
 ...
 local function onPressFlipForward( event )  
    composer.gotoScene( "rabbitPage", "flip", 200  )
...
local function  playAudio( event )
    -- loading audio
    for row in db:nrows("SELECT audio FROM domesticanimal WHERE id = '3'") do
...
{  
    ...
    defaultFile = "horseTitle.png",
    overFile = "horseTitle.png"
}
...
local function playVideo( event )
  ...
  media.playVideo( "rtsp://r3---sn-a5m7zu7l.c.youtube.com/CiILENy73wIaGQl95_Xc2tU2CxMYDSANFEgGUgZ2aWRlb3MM/0/0/0/video.3gp", media.RemoteSource, true )  
  -- body
end
...
local function  playSound( event )
    -- loading audio
    for row in db:nrows("SELECT sounds FROM domesticanimal WHERE id = '3'") do
