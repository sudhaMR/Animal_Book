...
local function onPressBack( self,event )  
    ...
    composer.removeScene("giraffePage")
  ...
  scrollScreen:insert(backButton)
     ...
     for row in db:nrows("SELECT description,image FROM wildanimalInfo WHERE id = '7'") do
...
local function onPressFlipBackward( event )  
    composer.gotoScene( "deerPage", "flip", 200  )
...
local function  playAudio( event )
    -- loading audio
    for row in db:nrows("SELECT audio FROM wildanimalInfo WHERE id = '7'") do
...
local myButton = widget.newButton
{
    ...
    defaultFile = "giraffeTitle.png",
    overFile = "giraffeTitle.png"  
}
...
local function playVideo( event )
  ...
  media.playVideo( "rtsp://r3---sn-a5m7zu7s.c.youtube.com/CiILENy73wIaGQnXUMmTy7qjuhMYESARFEgGUgZ2aWRlb3MM/0/0/0/video.3gp", media.RemoteSource, true )  
  -- body
end
...
local function  playSound( event )
    -- loading audio
    for row in db:nrows("SELECT sounds FROM wildanimalInfo WHERE id = '7'") do
