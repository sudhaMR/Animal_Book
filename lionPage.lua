...
local function onPressBack( self,event )	
  ...
  scrollScreen:insert(backButton)
     ...
     for row in db:nrows("SELECT description,image FROM wildanimalInfo WHERE id = '2'") do

      local lionImage = display.newImage(row.image)
      lionImage.x = display.contentCenterX
      lionImage.y = display.viewableContentHeight/1.8
      lionImage.height = display.viewableContentWidth -  display.viewableContentWidth/16
      lionImage.width =  display.viewableContentWidth -  2*display.viewableContentWidth/16
      sceneGroup:insert(lionImage)
      scrollScreen:insert(lionImage)	

      local options =
       {
        text = row.description, 
        x = display.contentCenterX,
        y = lionImage.y+ (lionImage.height/2) + (lionImage.height/4) + display.viewableContentHeight/2.7,
...
local function  playAudio( event )
    -- loading audio
    for row in db:nrows("SELECT audio FROM wildanimalInfo WHERE id = '2'") do
...
local myButton = widget.newButton
{
    ...
    defaultFile = "lionTitle.png",
    overFile = "lionTitle.png"  
}
...
local function playVideo( event )
  ...
  media.playVideo( "rtsp://r5---sn-a5m7zu7e.c.youtube.com/CiILENy73wIaGQmPO5eToJLEPBMYDSANFEgGUgZ2aWRlb3MM/0/0/0/video.3gp", media.RemoteSource, true )  
  -- body
end
...
local function  playSound( event )
    -- loading audio
    for row in db:nrows("SELECT sounds FROM wildanimalInfo WHERE id = '2'") do
