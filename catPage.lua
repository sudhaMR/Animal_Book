...
local function onPressBack( self,event )	
  ...
  scrollScreen:insert(backButton)
     ...
     for row in db:nrows("SELECT description,image FROM domesticanimal WHERE id = '2'") do


      local catImage = display.newImage(row.image)
      catImage.x = display.contentCenterX       
      catImage.y = display.viewableContentHeight/1.8
      catImage.height = display.viewableContentWidth -  display.viewableContentWidth/16
      catImage.width = display.viewableContentWidth -  2*display.viewableContentWidth/16
      sceneGroup:insert(catImage)
      scrollScreen:insert(catImage)

      local options =
       {
        text = row.description, 
        x = display.contentCenterX +  display.contentCenterX /16 ,
        y = catImage.y+ (catImage.height/2) + (catImage.height/4) + display.viewableContentHeight/2.7,
...
local function  playAudio( event )
    -- loading audio
    for row in db:nrows("SELECT audio FROM domesticanimal WHERE id = '2'") do
    soundFile = audio.loadSound(row.audio)
     audio.setVolume(1)
    audio.play(soundFile)    
    end
  --  media.playVideo( "rtsp://r1---sn-5hn7su7s.c.youtube.com/CiILENy73wIaGQlxlRxEX_T3jBMYESARFEgGUgZ2aWRlb3MM/0/0/0/video.3gp", media.RemoteSource, true )
    end
...
{  
    ...
    defaultFile = "catTitle.png",
    overFile = "catTitle.png"
}
...
local function  playSound( event )
    -- loading audio
    for row in db:nrows("SELECT sounds FROM domesticanimal WHERE id = '2'") do
