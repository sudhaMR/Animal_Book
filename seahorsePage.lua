...
local function onPressBack( self,event )  
    ...
    composer.removeScene("seahorsePage")
  ...
  scrollScreen:insert(backButton)
     ...
     for row in db:nrows("SELECT description,image FROM marineInfo WHERE id = '3'") do

     local seahorseImage = display.newImage(row.image)
      seahorseImage.x = display.contentCenterX       
      seahorseImage.y =  display.viewableContentHeight/1.8
      seahorseImage.height = display.viewableContentWidth -  display.viewableContentWidth/16
      seahorseImage.width = display.viewableContentWidth -  2*display.viewableContentWidth/16
      sceneGroup:insert(seahorseImage)
      scrollScreen:insert(seahorseImage)  
      ...
      local options =
       {
        ...
        y = seahorseImage.y + (seahorseImage.height/2) + (seahorseImage.height/4) + display.viewableContentHeight/2.7,
...
local function onPressFlipBackward( event )  
    composer.gotoScene( "turtlePage", "flip", 200  )   
    return true
  end
 ...
 local function onPressFlipForward( event )  
    composer.gotoScene( "octopusPage", "flip", 200  )    
    return true
  end
...
local function  playAudio( event )
    -- loading audio
    for row in db:nrows("SELECT audio FROM marineInfo WHERE id = '3'") do
...
{  
    ...
    defaultFile = "seahorseTitle.png",
    overFile = "seahorseTitle.png"
}
...
local function playVideo( event )
  ...
  media.playVideo( "rtsp://r8---sn-a5m7zu7l.c.youtube.com/CiILENy73wIaGQm3V9dymQnKdRMYDSANFEgGUgZ2aWRlb3MM/0/0/0/video.3gp", media.RemoteSource, true )  
  -- body
end
...
local function  playSound( event )
    -- loading audio
    for row in db:nrows("SELECT sounds FROM marineInfo WHERE id = '3'") do
