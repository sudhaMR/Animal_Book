...
local function onPressBack( self,event )  
    ...
    composer.removeScene("dolphinPage")
  ...
  scrollScreen:insert(backButton)
     ...
     for row in db:nrows("SELECT description,image FROM marineInfo WHERE id = '1'") do

     local dolphinImage = display.newImage(row.image)
      dolphinImage.x = display.contentCenterX       
      dolphinImage.y =  display.viewableContentHeight/1.8
      dolphinImage.height = display.viewableContentWidth -  display.viewableContentWidth/16
      dolphinImage.width = display.viewableContentWidth -  2*display.viewableContentWidth/16
      sceneGroup:insert(dolphinImage)
      scrollScreen:insert(dolphinImage)  
      ...
      local options =
       {
        ...
        y = dolphinImage.y + (dolphinImage.height/2) + (dolphinImage.height/4) + display.viewableContentHeight/2.7,
...
local function  playAudio( event )
    -- loading audio
    for row in db:nrows("SELECT audio FROM marineInfo WHERE id = '1'") do
...
{  
    ...
    defaultFile = "dolphinTitle.png",
    overFile = "dolphinTitle.png"
}
...
local function playVideo( event )
  ...
  media.playVideo( "rtsp://r8---sn-a5m7zu7l.c.youtube.com/CiILENy73wIaGQm7sXARplrCchMYDSANFEgGUgZ2aWRlb3MM/0/0/0/video.3gp", media.RemoteSource, true )  
  -- body
end
...
local function  playSound( event )
    -- loading audio
    for row in db:nrows("SELECT sounds FROM marineInfo WHERE id = '1'") do
