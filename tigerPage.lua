...
local function onPressBack( self,event )	
    ...
    composer.removeScene("tigerPage")
  ...
  scrollScreen:insert(backButton)
     ...
     for row in db:nrows("SELECT description,image FROM wildanimalInfo WHERE id = '1'") do

     local tigerImage = display.newImage(row.image)
      tigerImage.x = display.contentCenterX       
      tigerImage.y =  display.viewableContentHeight/1.8
      tigerImage.height = display.viewableContentWidth -  display.viewableContentWidth/16
      tigerImage.width = display.viewableContentWidth -  2*display.viewableContentWidth/16
      sceneGroup:insert(tigerImage)
      scrollScreen:insert(tigerImage)	
      ...
      local options =
       {
        ...
        y = tigerImage.y + (tigerImage.height/2) + (tigerImage.height/4) + display.viewableContentHeight/2.7,
...
local function onPressFlipBackward( event )  
    composer.gotoScene( "lionPage", "flip", 200  )   
    return true
  end
 ...
 local function onPressFlipForward( event )  
    composer.gotoScene( "elephantPage", "flip", 200  )    
    return true
  end
...
local function  playAudio( event )
    -- loading audio
    for row in db:nrows("SELECT audio FROM wildanimalInfo WHERE id = '1'") do
...
{  
    ...
    defaultFile = "tigerTitle.png",
    overFile = "tigerTitle.png"
}
...
local function playVideo( event )
  ...
  media.playVideo( "rtsp://r7---sn-a5m7zu7r.c.youtube.com/CiILENy73wIaGQmSw4ndabXxGhMYDSANFEgGUgZ2aWRlb3MM/0/0/0/video.3gp", media.RemoteSource, true )  
  -- body
end
...
local function  playSound( event )
    -- loading audio
    for row in db:nrows("SELECT sounds FROM wildanimalInfo WHERE id = '1'") do
