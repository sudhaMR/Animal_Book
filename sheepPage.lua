...
local function onPressBack( self,event )  
    ...
    composer.removeScene("sheepPage")
  ...
  scrollScreen:insert(backButton)
     ...
     for row in db:nrows("SELECT description,image FROM farmanimalInfo WHERE id = '3'") do               

      local sheepImage = display.newImage(row.image)
      --sheepImage.x = display.contentCenterX       
      --sheepImage.y =  display.viewableContentHeight - display.viewableContentHeight/56
      sheepImage.x = display.contentCenterX 
      sheepImage.y = display.viewableContentHeight/1.8
      --sheepImage.y = display.contentCenterY + display.contentCenterY/24
      sheepImage.height = display.viewableContentWidth -  display.viewableContentWidth/16
      sheepImage.width = display.viewableContentWidth -  2*display.viewableContentWidth/16
      sceneGroup:insert(sheepImage)
      scrollScreen:insert(sheepImage)
      ...
      local options =
       {
        ...
        --x = display.contentCenterX +  display.contentCenterX /16 ,
        x = display.contentCenterX,
        --y = display.viewableContentHeight/2,
        --left= display.viewableContentWidth,      
      	--y =  display.viewableContentHeight - display.viewableContentHeight/56,
     	y =  sheepImage.y+ (sheepImage.height/2) + (sheepImage.height/4) + display.viewableContentHeight/2.7 ,
      --  y =  sheepImage.y+ (sheepImage.height/2) + (sheepImage.height/4) + display.viewableContentHeight/8,
        font = "Verdana",
        fontSize = 20,
        align =  center
      }
...
local function onPressFlipBackward( event )  
    composer.gotoScene( "goatPage", "flip", 200  )   
    return true
  end
 ...
 local function onPressFlipForward( event )  
    composer.gotoScene( "pigPage", "flip", 200  )    
    return true
  end
...
local function  playAudio( event )
    -- loading audio
    for row in db:nrows("SELECT audio FROM farmanimalInfo WHERE id = '3'") do
...
{  
    ...
    defaultFile = "sheepTitle.png",
    overFile = "sheepTitle.png"
}
...
local function playVideo( event )
  ...
  media.playVideo( "rtsp://r4---sn-a5m7zu7k.c.youtube.com/CiILENy73wIaGQnm09m5U0BD7BMYESARFEgGUgZ2aWRlb3MM/0/0/0/video.3gp", media.RemoteSource, true )  
  -- body
end
...
local function  playSound( event )
    -- loading audio
    for row in db:nrows("SELECT sounds FROM farmanimalInfo WHERE id = '3'") do
