...
local function onPressBack( self,event )	
  ...
  scrollScreen:insert(backButton)
     ...
     for row in db:nrows("SELECT description,image FROM wildanimalInfo WHERE id = '4'") do

     local monkeyImage = display.newImage(row.image)
      monkeyImage.x = display.contentCenterX       
      monkeyImage.y =  display.viewableContentHeight/1.8
      monkeyImage.height = display.viewableContentWidth -  display.viewableContentWidth/16
      monkeyImage.width = display.viewableContentWidth -  2*display.viewableContentWidth/16
      sceneGroup:insert(monkeyImage)
      scrollScreen:insert(monkeyImage)	
      ...
      local options =
       {
        ...
        y = monkeyImage.y + (monkeyImage.height/2) + (monkeyImage.height/4) + display.viewableContentHeight/2.7,
...
local function  playAudio( event )
    -- loading audio
    for row in db:nrows("SELECT audio FROM wildanimalInfo WHERE id = '4'") do
...
{  
    ...
    defaultFile = "monkeyTitle.png",
    overFile = "monkeyTitle.png"
}
...
local function playVideo( event )
  ...
  media.playVideo( "rtsp://r4---sn-a5m7zu76.c.youtube.com/CiILENy73wIaGQkGlPyBDsTTeRMYDSANFEgGUgZ2aWRlb3MM/0/0/0/video.3gp", media.RemoteSource, true )  
  -- body
end
...
local function  playSound( event )
    -- loading audio
    for row in db:nrows("SELECT sounds FROM wildanimalInfo WHERE id = '4'") do
