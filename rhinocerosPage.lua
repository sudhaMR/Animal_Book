...
local function onPressBack( self,event )  
    ...
    composer.removeScene("rhinocerosPage")
  ...
  scrollScreen:insert(backButton)
     ...
     for row in db:nrows("SELECT description,image FROM wildanimalInfo WHERE id = '8'") do
...
local function onPressFlipBackward( event )  
  ...
  local function onPressFlipForward( event )  
    composer.gotoScene( "hippopotamusPage", "flip", 200  )
...
local function  playAudio( event )
    -- loading audio
    for row in db:nrows("SELECT audio FROM wildanimalInfo WHERE id = '8'") do
...
local myButton = widget.newButton
{
    ...
    defaultFile = "rhinocerosTitle.png",
    overFile = "rhinocerosTitle.png"  
}
...
local function playVideo( event )
  ...
  media.playVideo( "rtsp://r1---sn-a5m7zu7d.c.youtube.com/CiILENy73wIaGQkdFMD_UgCJ2hMYDSANFEgGUgZ2aWRlb3MM/0/0/0/video.3gp", media.RemoteSource, true )  
  -- body
end
...
local function  playSound( event )
    -- loading audio
    for row in db:nrows("SELECT sounds FROM wildanimalInfo WHERE id = '8'") do
