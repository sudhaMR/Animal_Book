...
local function onPressBack( self,event )  
    ...
    composer.removeScene("hippopotamusPage")
  ...
  scrollScreen:insert(backButton)
     ...
     for row in db:nrows("SELECT description,image FROM wildanimalInfo WHERE id = '9'") do
...
local function onPressFlipBackward( event )  
    composer.gotoScene( "rhinocerosPage", "flip", 200  )
...
local function  playAudio( event )
    -- loading audio
    for row in db:nrows("SELECT audio FROM wildanimalInfo WHERE id = '9'") do
...
local myButton = widget.newButton
{
    ...
    defaultFile = "hippopotamusTitle.png",
    overFile = "hippopotamusTitle.png"  
}
...
local function playVideo( event )
  ...
  media.playVideo( "rtsp://r5---sn-a5m7zu7l.c.youtube.com/CiILENy73wIaGQmSxo2rexmA3xMYDSANFEgGUgZ2aWRlb3MM/0/0/0/video.3gp", media.RemoteSource, true )  
  -- body
end
...
local function  playSound( event )
    -- loading audio
    for row in db:nrows("SELECT sounds FROM wildanimalInfo WHERE id = '9'") do
