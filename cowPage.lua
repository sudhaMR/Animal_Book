...
local function onPressBack( self,event )	
  ...
  scrollScreen:insert(backButton)
     ...
     for row in db:nrows("SELECT description,image FROM farmanimalInfo WHERE id = '1'") do

      local cowImage = display.newImage(row.image)
      cowImage.x = display.contentCenterX
      cowImage.y = display.viewableContentHeight/1.8
      cowImage.height = display.viewableContentWidth -  display.viewableContentWidth/16
      cowImage.width =  display.viewableContentWidth -  2*display.viewableContentWidth/16
      sceneGroup:insert(cowImage)
      scrollScreen:insert(cowImage)	

      local options =
       {
        text = row.description, 
        x = display.contentCenterX,
        y = cowImage.y+ (cowImage.height/2) + (cowImage.height/4) + display.viewableContentHeight/2.7,
...
local function  playAudio( event )
    -- loading audio
    for row in db:nrows("SELECT audio FROM farmanimalInfo WHERE id = '1'") do
...
local myButton = widget.newButton
{
    ...
    defaultFile = "cowTitle.png",
    overFile = "cowTitle.png"  
}
...
local function playVideo( event )
  ...
  media.playVideo( "rtsp://r8---sn-a5m7zu7r.c.youtube.com/CiILENy73wIaGQk0XfJvzLpG6BMYDSANFEgGUgZ2aWRlb3MM/0/0/0/video.3gp", media.RemoteSource, true )  
  -- body
end
...
local function  playSound( event )
    -- loading audio
    for row in db:nrows("SELECT sounds FROM farmanimalInfo WHERE id = '1'") do
