...
local function onPressBack( self,event )	
  ...
  scrollScreen:insert(backButton)
     ...
     for row in db:nrows("SELECT description,image FROM wildanimalInfo WHERE id = '5'") do
...
local function  playAudio( event )
    -- loading audio
    for row in db:nrows("SELECT audio FROM wildanimalInfo WHERE id = '5'") do
...
local myButton = widget.newButton
{
    ...
    defaultFile = "bearTitle.png",
    overFile = "bearTitle.png"  
}
...
local function playVideo( event )
  ...
  media.playVideo( "rtsp://r2---sn-a5m7zu7s.c.youtube.com/CiILENy73wIaGQka2tyqnLvUOhMYESARFEgGUgZ2aWRlb3MM/0/0/0/video.3gp", media.RemoteSource, true )  
  -- body
end
...
local function  playSound( event )
    -- loading audio
    for row in db:nrows("SELECT sounds FROM wildanimalInfo WHERE id = '5'") do
