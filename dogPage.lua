...
local function onPressBack( self,event )	
  ...
  scrollScreen:insert(backButton)
     ...
     for row in db:nrows("SELECT description,image FROM domesticanimal WHERE id = '1'") do

      local dogImage = display.newImage(row.image)
      dogImage.x = display.contentCenterX
      dogImage.y =  display.viewableContentHeight/1.8      
      dogImage.height = display.viewableContentWidth -  display.viewableContentWidth/16
      dogImage.width =  display.viewableContentWidth -  2*display.viewableContentWidth/16
      sceneGroup:insert(dogImage)
      scrollScreen:insert(dogImage)

      local options =
       {
        text = row.description, 
        x = display.contentCenterX,     
        y = dogImage.y+ (dogImage.height/2) + (dogImage.height/4) + display.viewableContentHeight/2.7,
...
local function  playAudio( event )
    -- loading audio
    for row in db:nrows("SELECT audio FROM domesticanimal WHERE id = '1'") do
...
local myButton = widget.newButton
{
    ...
    defaultFile = "dogTitle.png",
    overFile = "dogTitle.png"  
}
...
local function playVideo( event )
  ...
  media.playVideo( "rtsp://r4---sn-5hn7su7r.c.youtube.com/CiILENy73wIaGQlknRkbwlupORMYDSANFEgGUgZ2aWRlb3MM/0/0/0/video.3gp", media.RemoteSource, true )  
  -- body
end
...
local function  playSound( event )
    -- loading audio
    for row in db:nrows("SELECT sounds FROM domesticanimal WHERE id = '1'") do
