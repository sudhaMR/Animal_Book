...
local function onPressBack( self,event )	
    ...
    composer.removeScene("elephantPage")
  ...
  scrollScreen:insert(backButton)
     ...
     for row in db:nrows("SELECT description,image FROM wildanimalInfo WHERE id = '3'") do

     local elephantImage = display.newImage(row.image)
      elephantImage.x = display.contentCenterX       
      elephantImage.y =  display.viewableContentHeight/1.8
      elephantImage.height = display.viewableContentWidth -  display.viewableContentWidth/16
      elephantImage.width = display.viewableContentWidth -  2*display.viewableContentWidth/16
      sceneGroup:insert(elephantImage)
      scrollScreen:insert(elephantImage)	
      ...
      local options =
       {
        ...
        y = elephantImage.y + (elephantImage.height/2) + (elephantImage.height/4) + display.viewableContentHeight/2.7,
...
local function onPressFlipBackward( event )  
    composer.gotoScene( "tigerPage", "flip", 200  )   
    return true
  end
 ...
 local function onPressFlipForward( event )  
    composer.gotoScene( "monkeyPage", "flip", 200  )    
    return true
  end
...
local function  playAudio( event )
    -- loading audio
    for row in db:nrows("SELECT audio FROM wildanimalInfo WHERE id = '3'") do
...
{  
    ...
    defaultFile = "elephantTitle.png",
    overFile = "elephantTitle.png"
}
...
local function playVideo( event )
  ...
  media.playVideo( "rtsp://r2---sn-a5m7zu7z.c.youtube.com/CiILENy73wIaGQmbCWCMRKoMtxMYDSANFEgGUgZ2aWRlb3MM/0/0/0/video.3gp", media.RemoteSource, true )  
  -- body
end
...
local function  playSound( event )
    -- loading audio
    for row in db:nrows("SELECT sounds FROM wildanimalInfo WHERE id = '3'") do
