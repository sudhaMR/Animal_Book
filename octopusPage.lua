...
local function onPressBack( self,event )  
    ...
    composer.removeScene("octopusPage")
  ...
  scrollScreen:insert(backButton)
     ...
     for row in db:nrows("SELECT description,image FROM marineInfo WHERE id = '4'") do

     local octopusImage = display.newImage(row.image)
      octopusImage.x = display.contentCenterX       
      octopusImage.y =  display.viewableContentHeight/1.8
      octopusImage.height = display.viewableContentWidth -  display.viewableContentWidth/16
      octopusImage.width = display.viewableContentWidth -  2*display.viewableContentWidth/16
      sceneGroup:insert(octopusImage)
      scrollScreen:insert(octopusImage)  
      ...
      local options =
       {
        ...
        y = octopusImage.y + (octopusImage.height/2) + (octopusImage.height/4) + display.viewableContentHeight/2.7,
...
local function  playAudio( event )
    -- loading audio
    for row in db:nrows("SELECT audio FROM marineInfo WHERE id = '4'") do
...
{  
    ...
    defaultFile = "octopusTitle.png",
    overFile = "octopusTitle.png"
}
...
local function playVideo( event )
  ...
  media.playVideo( "rtsp://r4---sn-a5m7zu7r.c.youtube.com/CiILENy73wIaGQlScT9gLGm90hMYDSANFEgGUgZ2aWRlb3MM/0/0/0/video.3gp", media.RemoteSource, true )  
  -- body
end
...
local function  playSound( event )
    -- loading audio
    for row in db:nrows("SELECT sounds FROM marineInfo WHERE id = '4'") do
